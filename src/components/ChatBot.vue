...
export default{
  ...
  methods: {
    async initializeBot() {
      ...
      if(themes[0].intents){
        const optionsHtml = JSON.parse(themes[0].intents).map(option => `<button class="bot-option" data-intent-id="${option.key}" data-text="${option.name}" data-question="${option.name}" data-threshold="1.0" style="margin: unset;margin-top: .25rem;border: 1px solid #003366;color: #003366;transform-origin: 0;">${option.name}</button>`).join('');
          this.messages.push({
            text: optionsHtml,
            classes: ['message'],
            dataUser: false,
