<template>
  <div>
    <Navbar />
    <div class="actions">
      <h3>Virtualni asistenti za Ministarstvo pravosuđa, uprave i digitalne transformacije i digitalne transformacije</h3>
      <table>
        <thead>
          <tr>
            <th
            ...
            <th
              ...
              @click="toggleSortIcon(1, 'intentsCount')"
              :class="{ active: sortIcon[1] === 2 || sortIcon[1] === 3 }"
            >
              <div class="span-wrapper">
                <span>Broj namjera</span>
                <span>
                  <SortingIcon
                    :sortIcon="sortIcon[1]"
...
export default {
  components: { Navbar, SortingIcon },
  data() {
    return {
      chatbots: [],
      initialChatbotsOrder: [],
      sortIcon: [1, 1],
  ...
  methods: {
    ...
    async getSystems() {
      try {
        ...
        this.initialChatbotsOrder = [...this.chatbots];
      } catch (error) {
        console.error(error);
      }
    },
    ...
    toggleSortIcon(index, sortSubject) {
      if (this.sortIcon[index] === 1) {
        ...
        if (sortSubject === "name") {
          this.chatbots.sort((a, b) => a.name.localeCompare(b.name));
        } else if (sortSubject === "intentsCount")
          this.chatbots.sort((a, b) => a.intents_count - b.intents_count);
        ...
        this.sortIcon[index] = 1;
        this.chatbots = this.initialChatbotsOrder;
      }
    },
  },
};
