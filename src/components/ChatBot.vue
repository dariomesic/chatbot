...
export default {
  ...
  async mounted() {
    ...
    chatContainer.addEventListener("click", async (event) => {
      if (event.target.classList.contains("bot-option")) {
        ...
        if (
          !(
            event.target.getAttribute("data-text") == "PRETRAŽI BAZU ZNANJA" ||
            event.target.getAttribute("data-text") == "PREFORMULIRAT ĆU PITANJE"
          )
        ) {
          this.addUserMessage(event.target.getAttribute("data-text"));
          response = await JSON.parse(
            await DataService.getRulesForIntent(
              event.target.getAttribute("data-intent-id")
            )
          )[0];
          ...
          this.addUserMessage(event.target.getAttribute("data-text"));
          response = await DataService.searchDocuments(
            event.target.getAttribute("data-question")
          );
          let messageText = `<div><p>U našoj bazi pronašli smo sljedeći dokument s najvećim podudaranjem:</p><h4>${
            response.document_title
          }${
            response.document_page ? `(${response.document_page}.str)` : ""
          }</h4><div style="font-style:italic">${response.text}</div></div>`;
          this.addBotMessage({ assistant_answer: messageText });
        }
        await DataService.updateConversationTmp(
          this.sessionUUID,
          this.$route.query.system_id,
          Number(event.target.getAttribute("data-intent-id")),
          event.target.getAttribute("data-question"),
  ...
  methods: {
    async initializeBot() {
      ...
      if (themes[0].intents) {
        const optionsHtml = JSON.parse(themes[0].intents)
          .map(
            (option) =>
              `<button class="bot-option" data-intent-id="${option.key}" data-text="${option.name}" data-question="${option.name}" data-threshold="1.0">${option.name}</button>`
          )
          .join("");
        this.messages.push({
          text: optionsHtml,
          ...
          if (this.responseApi.response_type === "Slobodni tekst") {
            ...
            if (regEx.test(sanitizedInput)) {
              ...
              let messageText =
                `<div class="bot-response text" text-first="true">` +
                errorMessage +
                '</div><p class="time-text">' +
                new Date().toLocaleTimeString("en-US", { hour12: false }) +
                `</p>`;
              this.messages.push(
                {
                  text: '<svg style="width:20px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <rect x="3" y="11" width="18" height="10" rx="2"></rect> <circle cx="12" cy="5" r="2"></circle> <path d="M12 7v4"></path> <line x1="8" y1="16" x2="8" y2="16"></line> <line x1="16" y1="16" x2="16" y2="16"></line> </g></svg> <span style="margin-top:3px">ChatBot</span>',
    ...
    addUserMessage(message) {
      ...
      this.messages.push({
        text:
          `<div class="user-response">${message}</div><p class="time-text" style="text-align:end">` +
          new Date().toLocaleTimeString("en-US", { hour12: false }) +
          `</p>`,
        classes: ["message"],
        dataUser: true,
    ...
    async addBotMessage(message) {
      ...
      let messageText =
        `<div class="bot-response text" text-first="true"><pre>` +
        message.assistant_answer +
        `</pre></div><p class="time-text">` +
        new Date().toLocaleTimeString("en-US", { hour12: false }) +
        `</p>`;
    ...
    addPossibleIntents(message) {
      ...
      message.forEach((option) => {
        counter++;
        messageText += `<button class="bot-option" data-intent-id="${
          option.intent_id
        }" data-text="${option.intent_name}" data-question="${
          option.question
        }" data-threshold="${
          option.threshold
        }">${option.intent_name.toUpperCase()}</button>`;
        counter == 4
          ? (messageText += "<p style='text-align:center'>ili</p>")
          : "";
      });
      messageText +=
        '</div></div><p class="time-text">' +
        new Date().toLocaleTimeString("en-US", { hour12: false });
      +`</p>`;
      this.messages.push({
        text: messageText,
    ...
    removeLastOccurrence(inputString, update) {
      ...
      const afterSubstring = inputString.slice(
        lastIndex + "</pre></div>".length
      );
