      ...
      <div v-for="(new_value, index) in newValues" :key="index" class="synonym-group">
        ...
        <div class="synonym-info">
          <div class="input-container">
            <input @input="handleInput($event.target.value, index)" placeholder="Prazno polje" type="text" aria-describedby="" autocomplete="off" maxlength="128" :value="new_value" class="custom-input"/>
          ...
          <div class="old-values">
            ...
            <div contenteditable="true" @blur="handleBlur($event, index)" style="background: var(--background);padding-right: 2rem;scroll-margin-bottom: 2rem;width: 100%;padding: 10px;line-height: 17px;box-sizing: border-box;word-wrap: break-word;">
              {{ editedOldValues[index] }}
            </div>
          </div>
          <div>
            <button @click="deleteSynonyms(index)" style="align-items: center; display: inline-flex"><svg style="margin-top: 6px" focusable="false" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg" fill="currentColor" aria-label="Close" aria-hidden="true" width="16" height="16" viewBox="0 0 32 32" role="img" class="bx--btn__icon" > <path d="M24 9.4L22.6 8 16 14.6 9.4 8 8 9.4 14.6 16 8 22.6 9.4 24 16 17.4 22.6 24 24 22.6 17.4 16 24 9.4z" ></path> </svg></button>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async filterOldValues() {
      try {
        let synonyms = await DataService.getSynonyms(this.$route.query.system_id);
        ...
        if (Array.isArray(synonyms) && synonyms.length > 0) {
          ...
          this.editedOldValues = this.newValues.map(value => this.oldValuesMap.get(value).join(', '));
        ...
        console.log(this.newValues, this.oldValuesMap)
      } catch (error) {
    ...
    handleBlur(event, index) {
      ...
      const newValues = event.target.innerText.split(',').map(value => value.trim());
