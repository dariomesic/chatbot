<template>
  <div style="padding: 3.5rem 0 0 1rem;flex-grow: 1;overflow: auto;position: relative;">
        ...
        <div class="file-upload-container">
          ...
          <div class="uploaded-files-container" style="padding-left: 10rem;">
            ...
            <div v-for="document in documents" :key="document.id_doc" class="uploaded-file-item">
              <div style="display:flex;align-items: center;cursor:pointer;border-bottom: 1px solid black;padding-bottom: 5px;" @click="downloadDocument(document.id_doc, document.title.split('.'[1]))">
                <div class="file-icon">
    ...
    <Teleport to="body">
      <action-dialog
        :showDialog="showPreviousDialog"
        :rightButtonText="'Potvrda'"
        @close="showPreviousDialog = false"
        @confirm-action="goToPreviousVersion"
      >
...
export default {
  ...
  methods: {
    ...
    async deleteDocument(documentId) {
      try{
        this.loading = true
        await DataService.deleteDocument(documentId, this.$route.query.system_id);
        this.documents = await DataService.getDocumentsBySystemId(this.$route.query.system_id);
        this.show = true;
